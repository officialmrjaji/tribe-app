...
export function ActiveVoiceRoomProvider({ children }: { children: ReactNode }) {
  ...
  const [pendingSignOutLeave, setPendingSignOutLeave] = useState(false);
  ...
  }, [activeRoomId, clearActiveRoom, pendingSignOutLeave]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {chatUnreadCount > 0 ? (
          <span className="absolute -left-1 -bottom-1 flex min-h-6 min-w-6 items-center justify-center rounded-full border-2 border-white bg-[#f6c66f] px-1 text-[11px] font-bold text-[#17201b]">
